...
class NitterScraper(Scraper):
    ...
    async def __scrape_pages(self, page: Page) -> dict:
        result = {}
        for _ in range(10):
            page_result = await self.__scrape_one_page(page)
            result.update(page_result)
        return result

    async def __scrape_one_page(self, page: Page) -> dict:
        one_page_data = {}
        ...
        for item, content in zip(tweets_date, tweets_content):
            output = await self.__get_tweet_info(item, content)
    ...
    async def __get_tweet_info(self, item: Locator, content: Locator) -> dict:
        link = await item.get_attribute("href")
        username = await self.__get_username(link)
        ...
        date_time = await self.__transform_to_datetime(tweet_date)
    ...
    async def __transform_to_datetime(self, date: str) -> datetime:
        date_string = date.replace("·", "")
        timestamp = parser.parse(date_string)
        return timestamp

    async def __get_username(self, link) -> str:
        return link.split("/")[1]
